...
if [[ -z "$tempo_down" ]] && [[ -z "$grafana_down" ]] && [[ "$traceql_working" == "1" ]] && [[ "$metrics_working" == "1" ]] && [[ "$grafana_tempo_connected" == "1" ]]; then
...
if [[ "$integration_status" == "complete" ]]; then
    ...
    echo ""
    echo "1. ${BLUE}Explore Grafana Interface:${NC}"
    ...
    echo ""
    echo "2. ${BLUE}Run Demonstration Scripts:${NC}"
